...
class PickPlaceParams:
    ...
    # feedback correction clip (meters)
    xy_feedback_clip: float = 0.020
    ...
    path_speed_m_per_step: float = 0.00675  # 1.5× baseline (0.0045)
...
class WaypointPickPlacePlanner:
    ...
    def _build(self, env, eef_quat_xyzw: np.ndarray) -> None:
        ...
        self._plan_obj_xy = env.sim.data.body_xpos[env.object_A_body_id][:2].copy()
    ...
    def _apply_xy_feedback(self, env, pos: np.ndarray, phase: str) -> np.ndarray:
        ...
        clip = float(self.params.xy_feedback_clip)
        # During transfer / place, slowly track container XY drift vs plan.
        if phase in ("transfer", "place") and self._plan_cont_xy is not None:
            live = env.sim.data.body_xpos[env.object_B_body_id][:2].astype(np.float32)
            d = np.clip(live - self._plan_cont_xy.astype(np.float32), -clip, clip)
            pos[:2] = pos[:2] + d
        # Final approach: nudge toward object XY if it moved slightly.
        eef_xy = _eef_site_pos(env)[:2]
        if phase == "approach" and self._plan_obj_xy is not None:
            if float(np.linalg.norm(eef_xy - self._plan_obj_xy.astype(np.float32))) < 0.14:
                live_o = env.sim.data.body_xpos[env.object_A_body_id][:2].astype(np.float32)
                d2 = np.clip(live_o - self._plan_obj_xy.astype(np.float32), -clip, clip)
                pos[:2] = pos[:2] + d2
        return pos
    ...
    def _pose_to_action(
        ...
        if self._use_orientation_control and phase == "approach":
            ...
            # Lock gripper "upright": only allow rotation about base Z (yaw).
            rotvec_b[0] = 0.0
            rotvec_b[1] = 0.0
