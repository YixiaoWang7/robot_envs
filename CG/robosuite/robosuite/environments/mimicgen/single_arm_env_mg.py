...
class SingleArmEnv_MG(ManipulationEnv):
    ...
    def edit_model_xml(self, xml_str):
        ...
        path = os.path.join(os.environ["WORKING_ROOT"], "CG", "robosuite", "robosuite")
        ...
        for elem in all_elements:
            ...
            if len(check_lst) > 0:
                ind = max(check_lst) # last occurrence index
                new_path_split = os.path.join(os.environ["WORKING_ROOT"], "CG", "mimicgen").split("/") + old_path_split[ind + 1 :]
                new_path = "/".join(new_path_split)
                elem.set("file", new_path)
            ...
            if len(check_lst) > 0:
                ind = max(check_lst) # last occurrence index
                new_path_split = os.path.join(os.environ["WORKING_ROOT"], "CG", "robosuite-task-zoo").split("/") + old_path_split[ind + 1 :]
                new_path = "/".join(new_path_split)
                elem.set("file", new_path)
