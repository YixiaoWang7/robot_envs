...
from robosuite.models.objects import BallObject, BoxObject, CapsuleObject, Bin, Cup, Plate
...
class CG_L1(ManipulationEnv):
    """
    This class corresponds to the L1 task of CG benchmark, modified from Lift.
    ...
    Args:
        ...
        object_A_index (int): Index of the object A to be used in the task. 0: ball, 1: box, 2: capsule

        object_B_index (int): Index of the object B to be used in the task. 0: bin, 1: cup, 2: plate
    ...
    def __init__(
        ...
        object_A_index=1,  # 0: ball, 1: box, 2: capsule
        object_B_index=0,  # 0: bin, 1: cup, 2: plate
    ):
        ...
        # object selection
        self.object_A_index = object_A_index
        self.object_B_index = object_B_index
    ...
    def reward(self, action=None):
        ...
        Sparse un-normalized reward:
            ...
            - a discrete reward of 3.00 is provided if the cube is lifted

        Un-normalized summed components if using reward shaping:

            - Reaching: in [0, 1], to encourage the arm to reach the cube
            - Grasping: in {0, 0.5}, non-zero if arm is grasping the cube
            - Placing: in [0, 1], to encourage the arm to place the cube into the bin

        The sparse reward only consists of the lifting component.

        Note that the final reward is normalized and scaled by
        reward_scale / 4.00 as well so that the max score is equal to reward_scale
    ...
    def _load_model(self):
        ...
        self.objects = [self.object_A, self.object_B]
    ...
    def _check_success(self):
        ...
        if self.object_B_index == 0:  # Bin case
            ...
            z_check = (rel_pos[2] > -bin_size[2] / 2) and (rel_pos[2] < bin_size[2] / 2)
            
            return x_check and y_check and z_check

        elif self.object_B_index == 1:  # Cup case
            cup_inner_r = self.object_B.r1
            cup_height = self.object_B.height
            horizontal_dist = np.linalg.norm(rel_pos[:2])
            
            radius_check = horizontal_dist < cup_inner_r
            height_check = (rel_pos[2] > -cup_height / 2) and (rel_pos[2] < cup_height / 2)
            ...
            radius_check = horizontal_dist < plate_radius
            height_check = (rel_pos[2] > -plate_height / 2) and (rel_pos[2] < plate_height)
