...
def refactor_composite_controller_config(controller_config, robot_type, arms):
    ...
    config_dir = pathlib.Path("CG/robosuite/robosuite/controllers/config/robots/")
...
def load_composite_controller_config(controller: Optional[str] = None, robot: Optional[str] = None) -> Optional[Dict]:
    ...
    """
    # Resolve config paths relative to the installed robosuite package (do not rely on external env vars).
    # File is: robosuite/controllers/composite/composite_controller_factory.py
    _controllers_dir = pathlib.Path(__file__).resolve().parents[1]
    _config_root = _controllers_dir / "config"
    ...
    if controller is None:
        ...
        controller_fpath = _config_root / "robots" / f"default_{robot_name}.json"
        if not os.path.exists(controller_fpath):
            controller_fpath = _config_root / "default" / "composite" / "basic.json"
        ...
        if controller.endswith(".json"):
            ...
            controller_fpath = _config_root / "default" / "composite" / f"{controller_name}.json"
    else:
