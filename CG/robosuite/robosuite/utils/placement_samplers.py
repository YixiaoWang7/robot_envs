...
class UniformApartRandomSampler(UniformRandomSampler):
    """
    Modified version of UniformRandomSampler that ensures objects are placed a minimum distance apart from each other.
    """
    ...
    def sample(self, fixtures=None, reference=None, on_top=True, trial_times=5000):
        """
        Add min_distance constraint to the sampling process.
        """
        # Standardize inputs
        placed_objects = {} if fixtures is None else copy(fixtures)
        if reference is None:
            ...
            for i in range(trial_times):
                ...
                if self.ensure_valid_placement:
                    for (x, y, z), _, other_obj in placed_objects.values():
                        if (
                            np.linalg.norm((object_x - x, object_y - y))
                            <= other_obj.horizontal_radius + horizontal_radius + self.min_distance
                        ) and (object_z - z <= other_obj.top_offset[-1] - bottom_offset[-1]):
                            location_valid = False
                            break
