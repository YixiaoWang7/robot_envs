...
class StateBasedCGWrapper:
    ...
    def step(self, actions):
        ...
        for i, env in enumerate(self.envs):
            ...
            if self.is_success[i]:
                ...
                # tmp_actions = np.zeros_like(actions[i].copy())
                # tmp_actions[6] = 1
    ...
    def _compute_observation(self, idx, env=None):
        ...
        # 0.81535977
        # print(robot0_gripper_qpos)
        # print(robot0_eef_pos)
        # [-0.01183668  0.0118366 ] close
        # [ 0.01076607 -0.01076882] open
        # initial robot eef pose: [-0.08789131  0.00507217  1.01844344] [-0.10974402  0.02656894  0.98961622]
        
        # [ 0.03725395 -0.03726384]
        # [ 0.00050391 -0.00049568]
        ...
        # Match the state-policy training config: 9D robot state and 45D env_state.
        state = np.concatenate([robot0_eef_pos, robot0_eef_quat, robot0_gripper_qpos])
        gripper_to_object_A_pos = np.asarray(self.obs[idx]["gripper_to_object_A_pos"], dtype=np.float32)
        environment_state = np.concatenate([cross, cube, cylinder, bin, cup, plate, gripper_to_object_A_pos])
