...
BASE_CONFIGS = [
    ...
    os.path.join(BASE_BASE_CONFIG_PATH, "CG_L2.json"),
]
...
def make_generators(base_configs):
    ...
    all_settings = [
        ...
        # CG_L2
        dict(
            dataset_path=os.path.join(SRC_DATA_DIR, "CG_L2.hdf5"),
            dataset_name="CG_L2",
            generation_path="{}/CG_L2".format(OUTPUT_FOLDER),
            # task_interface="MG_PickPlace",
            tasks=["CG_L2"],
            task_names=["CG_L2"],
            select_src_per_subtask=True,
            ...
            # selection_strategy="nearest_neighbor_object",
            # selection_strategy_kwargs=dict(nn_k=3),
            subtask_term_offset_range=[[10, 20], [10, 20], [10, 20]],
        ),
    ]
