...
class MG_CG_L2(RobosuiteInterface):
    def update_instruction(self, instruction):
        self.env.update_instruction(instruction)
    ...
    def get_robot_eef_pose(self):
        ...
        # OSC control frame is a MuJoCo site - just retrieve its current pose
        return self.get_object_pose(
            obj_name="gripper0_right_grip_site", 
            obj_type="site",
        )
    ...
    def target_pose_to_action(self, target_pose, relative=True):
        ...
        # get maximum position and rotation action bounds
        max_dpos = self.env.robots[0].part_controllers['right'].output_max[0]
        max_drot = self.env.robots[0].part_controllers['right'].output_max[3]
    ...
    def get_object_poses(self):
        ...
        # four relevant objects - coffee pod, coffee machine, drawer, and mug
        return dict(
            cross=self.get_object_pose(obj_name=self.env.cross.root_body, obj_type="body"),
            cube=self.get_object_pose(obj_name=self.env.cube.root_body, obj_type="body"),
            cylinder=self.get_object_pose(obj_name=self.env.cylinder.root_body, obj_type="body"),
            bin=self.get_object_pose(obj_name=self.env.bin.root_body, obj_type="body"),
            cup=self.get_object_pose(obj_name=self.env.cup.root_body, obj_type="body"),
            plate=self.get_object_pose(obj_name=self.env.plate.root_body, obj_type="body"),
        )
    ...
    def get_subtask_term_signals(self):
        ...
        # checks which objects are in their correct bins and records them in @self.objects_in_bins
        total_success = self.env._check_success()

        # each subtask is a grasp and then a place
        signals["grasp"] = int(self.env._check_grasp(
            gripper=self.env.robots[0].gripper,
            object_geoms=[g for g in self.env.objects[self.env.object_A_index].contact_geoms])
        )
        
        # for obj_B_name, obj_B_id in object_B_names:
        signals["place"] = int(self.env.place_success())
