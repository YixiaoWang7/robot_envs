...
class CG_L2_Config(MG_Config):
    ...
    def task_config(self):
        ...
        '''
        i = 1
        for obj_A in ["cross", "cube", "cylinder"]:
            self.task.task_spec["subtask_{}".format(i)] = dict(
                object_ref=obj_A, 
                subtask_term_signal="grasp_{}".format(obj_A),
                subtask_term_offset_range=(10, 20),
                selection_strategy="nearest_neighbor_object",
                selection_strategy_kwargs=dict(nn_k=3),
                action_noise=0.05,
        ...
        for obj_B in ["bin", "cup", "plate"]:
            self.task.task_spec["subtask_{}".format(i)] = dict(
                object_ref=obj_B, 
                subtask_term_signal="place_{}".format(obj_B),
                subtask_term_offset_range=(10, 20),
                selection_strategy="nearest_neighbor_object",
                selection_strategy_kwargs=dict(nn_k=3),
                action_noise=0.05,
        ...
        self.task.task_spec["subtask_{}".format(i)] = dict( # Done
            object_ref=None,
            subtask_term_signal=None,
            subtask_term_offset_range=None,
            selection_strategy="random",
